...
static int add_query_edns(knot_pkt_t *packet, const query_t *query, uint16_t max_size)
{
	...
	edns_opt_t *custom_padding = NULL;

	query_t *q = (query_t *) query;
	dynarray_foreach(ednsopt, edns_opt_t *, option, q->ednsopt) {
		if (*option != NULL && !(*option)->cleared) {
			if ((*option)->code == KNOT_EDNS_OPTION_PADDING) {
				custom_padding = *option;
			} else {
	...
	bool align = query->padding != -3 && query->alignment > 0;
	if (align) {
	...
	if (padding > -1) {
		...
		// Use custom padding if set and alignment is not
		if (custom_padding && custom_padding->size > 0 && !align) {
...
{
	return query->edns > -1 || query->udp_size > -1 || query->flags.do_flag ||
		do_padding(query) || query->ednsopt.size > 0;
}
