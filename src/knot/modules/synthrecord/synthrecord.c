...
static int forward_addr_parse(knotd_qdata_t *qdata, synth_template_t *tpl,
                      ...
                      knot_rrset_t *rr, int addr_family)
{
	...
	knot_dname_t *ptrname = synth_ptrname(addr_str, tpl, addr_family);
	if (ptrname == NULL) {
		return KNOT_ENOMEM;
	}
