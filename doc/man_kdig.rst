...
**+**\ [\ **no**\ ]\ **ednsopt**\[\ =\ *CODE*\[:*VALUE*\]\]
  Specify EDNS option with option code and optionally payload of value
  as a hexadecimal string.+noednsopt[=CODE] clears the EDNS option(s)
  to to be sent. Nsid, padding and subnet are edns options with additional
  input method. Using ednsopt for padding allows padding with nonzero values
  and is prioritized over +padding=N.
